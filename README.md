...
Measured, not guessed. Same task, done twice: once with Claude doing the work directly, once with Claude only launching and reviewing while a local model did the work.

| | Done directly | Via a local model |
|---|---|---|
| Claude's tool calls | 6 | 3 |
| Characters Claude generated | ~1,874 | 721 (just the task description) |

The gap isn't the tool-call count so much as what those calls cost. The direct path is almost entirely generation, code Claude wrote character by character. The delegated path is mostly reading a diff back, plus one task description that doesn't get longer just because the underlying job does. Write the same size of task description for something five times more involved, and the direct path's cost grows with it while this one barely moves. That's the part expected to hold up at scale; there's no in-conversation way to check real dollar cost, only Claude Code's own `/cost` for that.

It isn't free money, though. The same setup, run against a smaller model on a slightly harder task, silently produced broken output and reported success anyway (see the findings section). Catching that costs a review cycle, sometimes a full do-over, which erases the savings for that run. The realistic version of this pitch: savings scale well when the model picked for the job is actually up to the job, and `foreman-recommend` exists specifically to make that less of a guess.
...
**Fixed:**
- A model marketed specifically for agentic coding (`devstral:24b`) never actually invoked structured tool-calling. It narrated tool calls as JSON text instead, which is a strange hill for an agentic coding model to die on. Dropped entirely; not worth chasing further.
